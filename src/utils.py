...
class Vocab:
    def __init__(self, actions_file, labels_file, pos_file, word_file):
        """'
        Initialize vocab lookup dictionaries
        """
        self.action_id = self.create_vocab(actions_file)
        self.label_id = self.create_vocab(labels_file)
        self.pos_id = self.create_vocab(pos_file)
        self.word_id = self.create_vocab(word_file)
    ...
    def word2id(self, word):
        return self.word_id[word] if word in self.word_id else self.word_id['<unk>']
        ...
        if not tok or line.strip() == '':
            ...
            if line[0] == '#' or '-' in tok[0] or '.' in tok[0]:
                ...
                tokens.append(DependencyToken(int(tok[0]), tok[1], tok[2], tok[3], tok[4], tok[5],
                                              ...
                                              '_' if test else tok[7], tok[8], tok[9]))
    if len(tokens) > 1:
        yield tokens
